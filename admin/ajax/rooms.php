<?php
    ...
    if(isset($_POST['add_room']))
    {
        ...
        $q2 = "INSERT INTO `room_facilities`(`room_id_type`, `facilities_id`) VALUES (?,?)";
        ...
        $q3 = "INSERT INTO `room_features`(`room_id_type`, `features_id`) VALUES (?,?)";
    ...
    if(isset($_POST['get_room']))
    {
        ...
        $res2 = select("SELECT * FROM `room_features` WHERE `room_id_type`=?",[$frm_data['get_room']],'i');
        $res3 = select("SELECT * FROM `room_facilities` WHERE `room_id_type`=?",[$frm_data['get_room']],'i');
    ...
    if(isset($_POST['edit_room']))
    {
        ...
        $del_features = delete("DELETE FROM `room_features` WHERE `room_id_type`=?", [$frm_data['room_id_type']],'i');
        $del_facilities = delete("DELETE FROM `room_facilities` WHERE `room_id_type`=?", [$frm_data['room_id_type']],'i');
        ...
        {
            foreach($facilities as $f){
                mysqli_stmt_bind_param($stmt,'ii',$frm_data['room_id_type'],$f);
                mysqli_stmt_execute($stmt);
            }
            $flag = 1;
            mysqli_stmt_close($stmt);
    ...
    if(isset($_POST['get_room_images']))
    {
        ...
        while($row = mysqli_fetch_assoc($res))
        {
            if($row['thumb']==1){
                ...
                $thumb_btn = "<button onclick='thumb_image($row[ad_id],$row[room_id])' class='btn btn-secondary shadow-none'>
            ...
            echo<<<data
                <tr class='align-middle'>
                    ...
                    <td>$thumb_btn</td>
                    <td>
                        <button onclick='rem_image($row[ad_id],$row[room_id])' class='btn btn-danger shadow-none'>
                            <i class='bi bi-trash'></i>
                        </button>
                    </td>
                </tr>
    ...
    if(isset($_POST['rem_image']))
    {
        ...
        $pre_q = "SELECT * FROM `room_images` WHERE `ad_id`=? AND `room_id`=?";
        $res = select($pre_q, $values, 'ii');
        $img = mysqli_fetch_assoc($res);

        if(deleteImage($img['image'],ROOMS_FOLDER)){
            $q = "DELETE FROM `room_images` WHERE `ad_id`=? AND `room_id`=?";
    ...
    if(isset($_POST['thumb_image']))
    {
        ...
        $q = "UPDATE `room_images` SET `thumb`=? WHERE `ad_id`=? AND `room_id`=?";
    ...
    if(isset($_POST['remove_room']))
    {
        ...
        $res3 = delete("DELETE FROM `room_features` WHERE `room_id_type`=?",[$frm_data['room_id_type']],'i');
        $res4 = delete("DELETE FROM `room_facilities` WHERE `room_id_type`=?",[$frm_data['room_id_type']],'i');
